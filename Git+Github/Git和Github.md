...
4. **提交文件到Git仓库(head)**
   ...
   注意，如果你提交了大文件，那么仓库也会记录，这可能会仓库内存太大导致推送远程仓库失败。你需要在历史记录中删除这个大文件，或者重置历史记录。
...
4. 重置git历史记录：
   ...
   ```bash
   # 1. 重置到第一个提交（清空所有历史）
   git update-ref -d HEAD  #删除HEAD引用，相当于删除当前分支的所有提交历史。（不会删除您的文件）
   
   # 2. 添加所有当前文件
   git add .   
   
   # 3. 创建全新的初始提交
   git commit -m "全新开始：清理历史"
   
   # 4. 强制推送到远程
   git push -f    # -f表示强制推送，用本地的全新历史覆盖远程仓库的所有历史
   
   # 注意，如果其他人在您强制推送之后执行 git pull，他们的本地历史会被搞乱。如果想要将远程历史完全覆盖本地历史，可以使用下面方法。
   git fetch origin   # 从远程仓库下载所有最新数据（不会改变本地的文件状态）
   git reset --hard origin/main  # --hard表示完全丢弃所有本地更改和历史，让本地仓库变得和远程一模一样（包括：文件内容、提交历史、所有状态）
   ```

   

5.  `.gitignore` 是 Git 版本控制系统中的一个配置文件，用于指定哪些文件或目录应该被 Git 忽略，不被纳入版本控制。基本语法规则：
   ...
   *.log
   ...
   # 通配符匹配
   *.tmp          # 所有 .tmp 文件
   temp?.*        # temp1.txt, temp2.js 等
   
   # 目录匹配
   logs/**/*.log  # logs 目录下所有子目录的 .log 文件
   ```
